...
routes_dir = r"c:\Aasritha\AgriTwin\Frontend\src\routes"

pages = [
    ("farm-overview", "Farm Overview", "MapPin"),
    ("live-monitoring", "Live Monitoring", "Activity"),
    ("disease-detection", "Disease Detection", "Microscope"),
    ("pest-monitoring", "Pest Monitoring", "Bug"),
    ("nutrient-analysis", "Nutrient Analysis", "FlaskConical"),
    ("water-management", "Water Management", "Droplets"),
    ("drone-monitoring", "Drone Monitoring", "Plane"),
    ("weather-intelligence", "Weather Intelligence", "CloudSun"),
    ("yield-prediction", "Yield Prediction", "TrendingUp"),
    ("harvest-planner", "Harvest Planner", "Wheat"),
    ("ai-crop-doctor", "AI Crop Doctor", "Bot"),
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-2 rounded-2xl border border-border bg-card shadow-sm p-6 min-h-[400px] flex items-center justify-center relative overflow-hidden group">
            ...
            <div className="text-center relative z-10">
                <__ICON__ className="h-16 w-16 text-muted-foreground/30 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-foreground">Interactive __TITLE__ Visualization</h3>
...
for path, title, icon in pages:
    ...
    content = template.replace("__PATH__", path).replace("__TITLE__", title).replace("__ICON__", icon).replace("__COMPONENT_NAME__", comp_name)
    with open(os.path.join(routes_dir, f"{path}.tsx"), "w", encoding="utf-8") as f:
        f.write(content)

print("Generated all 13 route files.")
